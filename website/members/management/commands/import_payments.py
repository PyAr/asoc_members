...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for payment in payments:
            ...
            payment_instance, created = Payment.objects.get_or_create(
                ...
                defaults={
                    "comments": template.format(filename, payment["comment"]),
                    "strategy": strategy,
                },
            )
    ...
    def get_patron(self, payment):
        patron = None
        patron_query = Patron.objects.filter(
            email=payment["strategy"]["patron"]["email"]
        )
        if patron_query.exists():
            ...
            document_number = (
                payment["strategy"]["patron"]["comment"].split("DNI")[-1].strip()
            )
